...
| # | 任务 | 类型 | 难度 | 状态 |
|:---:|:---|:---|:---:|:---:|
| T1 | ChnSentiCorp 中文情感二分类 | 文本分类 | ⭐⭐ | ✅ 三件套冻结，冒烟测试通过，Phase 1 就绪 |
...
| fixtures 隔离（workspace 无 gold_label） | ✅ | `input/` 无 gold |
...
### 测试 5：端到端三次验证（2026-08-31）

在 7766 条全量 gold 数据上，进行三次独立端到端验证，验证系统稳定性、评分一致性与 median_of_3 容错机制。

| 次序 | Judge 3次成功 | status | Judge分 | 总分 | 关键特征 |
|:---:|:---:|:---:|:---:|:---:|:---|
| 第 1 次 | 3/3 | OK | 6.49/10 | 96.49/100 | 全成功，无瞬断 |
| 第 2 次 | 3/3 | OK | 6.60/10 | 96.60/100 | 全成功，无瞬断 |
| 第 3 次 | 2/3 | PARTIAL_ERROR | 6.08/10 | 96.08/100 | 第 3 次网络瞬断，median_of_3 兜底 |

**关键发现**：
- **自动分稳定性**：三次均为 90.00/90，Oracle 层完全确定性
- **Judge 分自然变异**：6.08–6.60，变异系数 4.2%，证明即使 temperature=0 单次评分仍不可靠，median_of_3 设计有统计学意义
- **容错机制验证**：前两次证明网络稳定时 3/3 全成功；第三次证明网络瞬断时 median_of_3 自动兜底，2 次成功即可产出有效中位数
- **总分一致性**：96.08–96.60，变异 < 0.6%，系统可复现

详细记录见 `smoke_test_results/test5_triple_log.txt`。
...
> T1 Phase 1 预实验待启动（4 条件 × 10 遍 = 40 次）。
...
- [x] 目录结构修复 + 测试 5 三次验证通过（2026-08-31）
